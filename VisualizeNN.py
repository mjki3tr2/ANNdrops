# This library is modified based on https://github.com/jzliu-100/visualize-neural-network
...
class NeuralNetwork():
    ...
    def draw(self, weights_list=None, filename=None, figsize=None, ilabels=None, olabels=None):
        ...
        if filename is None:
            filename = 'ANN_' + strftime("%Y%m%d_%H%M%S", localtime()) + '.png'
...
class DrawNN():
    ...
    def draw( self , filename=None, figsize=None, ilabels=None, olabels=None):
        ...
        for l in self.neural_network:
            network.add_layer(l)
        if figsize is None:
            width = widest_layer * 0.5  # 0.5 inch per neuron
